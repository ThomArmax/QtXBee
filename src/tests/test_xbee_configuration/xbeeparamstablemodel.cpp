    ...
    m_xbee(bee)
{
    ...
    connect(m_xbee, SIGNAL(receivedATCommandResponse(ATCommandResponse*)), this, SLOT(onReceivedATCommandResponse(ATCommandResponse*)));
...
QVariant XBeeParamsTableModel::data(const QModelIndex &index, int role) const
{
    ...
    if(role == Qt::DisplayRole)
        return m_list.at(index.row())->data(index.column()).toByteArray();
...
void XBeeParamsTableModel::onReceivedATCommandResponse(ATCommandResponse *rep) {
    ...
    if(param) {
        ...
        param = new XBeeParamItem(ATCommand::atCommandToString(atcmd), data);
        param->setAtCommand(atcmd);
    }
    appendRow(param);
}
